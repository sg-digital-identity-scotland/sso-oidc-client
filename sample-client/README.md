...
There is a script to run in Docker, which requires Docker desktop.
...
JWT and adds it to token requests.
...
Important:
  ...
  - For local development, the redirect uri must have the structure `{PROTOCOL}://{HOST}:{PORT}/login/oauth2/code`.
    The `PROTOCOL` can be either `http` or `https`.
    The `HOST` can be `localhost` or a virtual host that can be added to the operating system's `/etc/hosts` file.
    Please Note that the authorization server will only allow URIs to be registered with a `http` if the `HOST` is set to `localhost`.
    If using a `HOST` other than `localhost`, please use `https`. 
...
Please also update the values for `PROTOCOL`, `HOST` and `PORT` in the `env` file with the values you used during client registration.

### 2 Generate key store

If the `PROTOCOL` value mentioned above has been set to `https` it will be necessary to generate a keystore as the client will need to be run on https.
The script `scripts/generate_keystore.sh` will generate a keystore 
...
### 3 Scopes
In the `env` file the `SCOPE` parameter has been set to `openid`. This should be the only scope required to complete the authentication journey.
Please only update this variable if the dis team has asked you to use extra scopes. Extra scopes can be added as a comma separate list to this variable e.g `SCOPE=openid,scope1,scope2`.
...
## Using the client
The client runs on `{PROTOCOL}://${HOST}:${PORT}`. `PROTOCOL`, `HOST` and `PORT` are set in `env`.
If running locally with `PROTOCOL` set to the `https` scheme, the browser may initially 
...
Once a user is successfully authenticated, the DIS authorization server redirects 
...
Once authenticated 2 extra links will appear on the home page `Logout` and `Refresh Access Token if Expired`. Their functionality is explained in separate sections below.
...
`chrome://settings/content/all?searchSubpage=.sign-in.service.gov.scot` and delete that cookie.

### Refresh Access Token if Expired

Calling the token endpoint returns a refresh token along with an access and id token.
The refresh token is sent to the token endpoint to obtain a new access token when the access token is about to expire. This is done automatically by the underlying `Spring OAuth2 client library`. It has handled by the `RefreshTokenReactiveOAuth2AuthorizedClientProvider` class whenever `OAuth2AuthorizedClient.getAccessToken()` is called.
By default, the Access token will be refreshed if it is due to expire in the next 60 seconds.

Clicking the `Refresh Access Token if Expired` link in the browser will call `OAuth2AuthorizedClient.getAccessToken()` which will perform the exchange of a refresh token for a new access token, if the access token is about to expire. However, nothing will be visible in the browser when this button is clicked.
...
The `BACKCHANNEL_LOGOUT_URI` is called every time a user logs out from the client. On a successful
...
If the private client is running on localhost, it will not be possible
